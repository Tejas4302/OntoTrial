...
function snowHeaders(pat){return {'Authorization':`Bearer ${pat}`,'X-Snowflake-Authorization-Token-Type':'PROGRAMMATIC_ACCESS_TOKEN','Content-Type':'application/json','Accept':'application/json','User-Agent':'OntoTrail/1.1'};}
...
async function executeSql(base,pat,warehouse,sql){
  const response=await fetch(`${base}/api/v2/statements`,{method:'POST',headers:snowHeaders(pat),body:JSON.stringify({statement:sql,warehouse,timeout:30})});
  let body=await response.json().catch(()=>({}));
  if(response.status===202&&body.statementHandle){
    for(let i=0;i<6;i++){
      await new Promise(r=>setTimeout(r,750));
      const poll=await fetch(`${base}/api/v2/statements/${encodeURIComponent(body.statementHandle)}`,{headers:snowHeaders(pat)});
...
export default async function handler(req,res){
  ...
  try{
    const analyst=await fetch(`${base}/api/v2/cortex/analyst/message`,{
      method:'POST',headers:snowHeaders(pat),
      body:JSON.stringify({messages:[{role:'user',content:[{type:'text',text:question}]}],semantic_view:semanticView,stream:false})
    });
    const body=await analyst.json().catch(()=>({}));
    if(!analyst.ok)return send(res,502,{error:body.message||body.error||`Snowflake Cortex Analyst returned ${analyst.status}.`,requestId:body.request_id});
    const parsed=normalizeAnalyst(body);let result=null;let executionWarning='';
    const sql=safeSelect(parsed.sql);
    if(sql){try{result=await executeSql(base,pat,warehouse,sql);}catch(err){executionWarning=err.message||'The generated SQL could not be executed.';}}
    return send(res,200,{source:'snowflake-cortex-analyst',requestId:body.request_id||analyst.headers.get('x-snowflake-request-id')||'',text:parsed.text,sql:sql||'',suggestions:parsed.suggestions,result,executionWarning});
  }catch(err){return send(res,502,{error:err?.message||'Could not reach Snowflake Cortex Analyst.'});}
}
