...
import {readDecisions,writeDecisions,nextDecisionId} from './services/decisions.js';
...
const navIcons={overview:'grid',orders:'orders',network:'network',scenarios:'sliders',evidence:'file',decisions:'check',about:'info'};
...
function decisionDialog(seed={}){
 ...
 openDetail(`<div class="dialog-head"><div><span class="eyebrow">DECISION WORKFLOW</span><h2 id="detail-title">Create decision</h2><p>Turn an insight into an owned, trackable action.</p></div><button class="icon-button" data-close="detail-dialog" aria-label="Close">${icon('close')}</button></div><form id="decision-form" class="detail-body"><label class="field-label">Decision title<input class="text-input" name="title" required maxlength="120" value="${e(seed.title||'')}"></label><label class="field-label">Problem / evidence<textarea class="text-input decision-textarea" name="problem" required maxlength="500">${e(seed.problem||'')}</textarea></label><label class="field-label">Recommended action<textarea class="text-input decision-textarea" name="action" required maxlength="500">${e(seed.action||'')}</textarea></label><div class="decision-form-grid"><label class="field-label">Owner<input class="text-input" name="owner" required maxlength="80" value="${e(seed.owner||'Supply Planning')}"></label><label class="field-label">Due date<input class="text-input" name="due" type="date" required value="${e(due)}"></label><label class="field-label">Priority<select class="text-input" name="priority"><option>High</option><option ${seed.priority==='Medium'?'selected':''}>Medium</option><option ${seed.priority==='Low'?'selected':''}>Low</option></select></label><label class="field-label">Status<select class="text-input" name="status"><option>Proposed</option><option ${seed.status==='Open'?'selected':''}>Open</option><option ${seed.status==='In progress'?'selected':''}>In progress</option><option>Blocked</option><option>Done</option></select></label></div><label class="field-label">Expected outcome<input class="text-input" name="expectedImpact" maxlength="180" value="${e(seed.expectedImpact||'')}"></label><input type="hidden" name="source" value="${e(seed.source||'Manual decision')}"><div class="form-actions"><button type="button" class="secondary" data-close="detail-dialog">Cancel</button><button class="primary">Create decision</button></div></form>`);
}
function createDecisionFromInsight(button){const pair=button.closest('.chat-pair');const q=pair?.querySelector('.user-question')?.textContent?.trim()||'Cortex insight';const a=pair?.querySelector('.direct-answer')?.textContent?.trim()||'';decisionDialog({title:q.slice(0,110),problem:a,action:'Define and approve the mitigation action based on this analysis.',owner:'Supply Planning',priority:'High',status:'Proposed',expectedImpact:'Reduce modeled supply-chain exposure',source:'Cortex Analyst'});}
...
const actions={
 'go-scenarios':()=>navigate('scenarios'),'go-orders':()=>navigate('orders'),'go-decisions':()=>navigate('decisions'),'new-decision':()=>decisionDialog(),'go-about':()=>navigate('about'),'go-overview':()=>navigate('overview'),
...
document.addEventListener('click',event=>{const target=event.target.closest('button,a');if(!target)return;
 try{
  ...
  if(target.dataset.decisionStatus){const item=decisions.find(d=>d.id===target.dataset.decisionStatus);if(item){item.status=target.dataset.nextStatus;item.updatedAt=new Date().toISOString();persistDecisions();render();toast(`Decision ${item.id} moved to ${item.status}.`);}return;}
...
document.addEventListener('submit',event=>{event.preventDefault();const form=event.target;
 try{
  ...
  if(form.id==='decision-form'){const fd=new FormData(form);const item={id:nextDecisionId(decisions),title:String(fd.get('title')||'').trim(),problem:String(fd.get('problem')||'').trim(),action:String(fd.get('action')||'').trim(),owner:String(fd.get('owner')||'').trim(),due:String(fd.get('due')||''),priority:String(fd.get('priority')||'Medium'),status:String(fd.get('status')||'Proposed'),expectedImpact:String(fd.get('expectedImpact')||'').trim(),scenario:scenarioSummary(scenario,d),source:String(fd.get('source')||'Manual decision'),createdAt:new Date().toISOString()};if(!item.title||!item.problem||!item.action||!item.owner||!item.due)throw Error('Complete the required decision fields.');decisions.unshift(item);persistDecisions();log(`Created decision ${item.id}: ${item.title}`);closeDialog('detail-dialog');navigate('decisions');toast('Decision created and added to the board.');return;}
