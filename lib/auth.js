...
export function credentials(email,password){
  ...
  if(e===clientEmail&&p===String(process.env.ONTOTRAIL_CLIENT_PASSWORD||''))return {role:'client',name:'CoCo CLI Hackathon User',tenant:'CoCo CLI Hackathon'};
  const adminEmail=String(process.env.ONTOTRAIL_ADMIN_EMAIL||'').trim().toLowerCase();
  if(adminEmail&&e===adminEmail&&p===String(process.env.ONTOTRAIL_ADMIN_PASSWORD||''))return {role:'admin',name:'OntoTrail Administrator',tenant:'OntoTrail'};
  return null;
}
