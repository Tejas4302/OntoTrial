...
export function credentials(email,password){
  ...
  if(clientAdminEmail&&e===clientAdminEmail&&p===String(process.env.ONTOTRAIL_CLIENT_ADMIN_PASSWORD||''))return {role:'client_admin',name:'Nova Mobility India Admin',tenant:'Nova Mobility India',email:e};
  const clientEmail=String(process.env.ONTOTRAIL_CLIENT_EMAIL||'').trim().toLowerCase();
  if(e===clientEmail&&p===String(process.env.ONTOTRAIL_CLIENT_PASSWORD||''))return {role:'client_user',name:'Nova Mobility India User',tenant:'Nova Mobility India',email:e};
  return null;
}
