...
export function credentials(email,password){
  ...
  if(platformEmail&&e===platformEmail&&p===String(process.env.ONTOTRAIL_ADMIN_PASSWORD||''))return {role:'platform_admin',name:'OntoTrail Administrator',tenant:'OntoTrail'};
  const clientAdminEmail=String(process.env.ONTOTRAIL_CLIENT_ADMIN_EMAIL||'').trim().toLowerCase();
  if(clientAdminEmail&&e===clientAdminEmail&&p===String(process.env.ONTOTRAIL_CLIENT_ADMIN_PASSWORD||''))return {role:'client_admin',name:'CoCo CLI Hackathon Admin',tenant:'CoCo CLI Hackathon'};
  const clientEmail=String(process.env.ONTOTRAIL_CLIENT_EMAIL||'').trim().toLowerCase();
  if(e===clientEmail&&p===String(process.env.ONTOTRAIL_CLIENT_PASSWORD||''))return {role:'client_user',name:'CoCo CLI Hackathon User',tenant:'CoCo CLI Hackathon'};
  return null;
}
...
export function readSession(req){
  ...
  try{const data=JSON.parse(dec(payload));if(!data?.exp||Date.now()>data.exp)return null;return {role:data.role,name:data.name,tenant:data.tenant};}catch{return null;}
}
